...
export async function runPipeline(
  ...
  console.info(
    `[pipeline] lexicons: ${Date.now() - t0}ms (${lexicons.length} items)`,
  );

  const replaceableLexicons = lexicons.filter((l) =>
    REPLACEABLE_TYPES.has(l.type),
  );
  ...
  const includeRanges = replaceableLexicons.map((l) => ({
    start: l.start,
    end: l.end,
  }));
  const t1 = Date.now();
